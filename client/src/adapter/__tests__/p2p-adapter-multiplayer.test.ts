...
describe("P2P wire-protocol version gate", () => {
  ...
  // refused, and the v56 frame stops being admitted. The admitting half is
  // the reach-guard — without it "refuses v53" is also satisfied by a client
  // that refuses everything.
  it("refuses the previous wire protocol (v55) and admits its own (v56)", async () => {
    const refusing = makeGuest();
    await refusing.adapter.initialize();
    await refusing.conn.simulateData(setupFrameAt(55));
    ...
    await admitting.conn.simulateData(setupFrameAt(56));
