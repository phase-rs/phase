...
- **Checkpoint mode's receipt-validation and artifact rules are untouched** — phase mode scopes what counts as a finding, never what counts as valid provenance. The supplied `BASE_SHA` is the phase base, and the reviewed span is that phase's `BASE_SHA..CANDIDATE_SHA`.
