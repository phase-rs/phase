...
- **Review:** stop after **three unsuccessful review attempts within one review loop**. Each review loop carries its own allowance of three: the sizing-audit loop, the charter loop, each phase's (or the unphased run's) plan loop and implementation-review loop, and the integration loop. Attempts are never summed across loops, and a loop restarted or renamed for the same work keeps that loop's tally. A phase added, split, merged or re-scoped by a decision revision starts its own loops at zero; that revision is counted in the charter loop. Findings requiring a revision count as an unsuccessful attempt; abandoning a dispatched review also counts. New findings count just as repeated ones do. Clean intermediate reviews consume no attempt and erase none. Charter corrections that require no re-review remain clean; review-only revisions requiring a fresh review count. Returning from implementation to planning re-enters that phase's plan loop with the history that loop already has. Only accepting a candidate that implements part of the original requested product behavior resets every loop's tally; a checkpoint, clean plan, helper-only phase or new reviewer does not. If tooling itself is the user's requested product, its accepted implementation qualifies.
