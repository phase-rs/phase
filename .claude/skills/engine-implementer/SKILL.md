...
**Charter revision** has two classes, told apart by one question: does the edit change a decision?

- **Decision revisions** may only add, split, merge, re-order, or re-scope **remaining** phases, and run through the charter-mode planner and the charter review loop. Accepted phases are never reworked in place: a finding that invalidates accepted content becomes a **fix phase** — a later phase whose scope overlaps the earlier files.
- **Administrative revisions** change no decision: extending a phase's `SCOPE_PATHS` by a file in one of the scope rule's three standing classes; replacing a code-state assertion with the claim the phase must establish, when the finding that names it supplies the replacement sentence; correcting a figure, a citation, or wording. The orchestrator applies these itself as check-and-replace edits with two-sided verification per edit (the old text present at the named coordinate before; the new text present exactly once after) and a sweep of the neighbours the edit breaks — the mechanics surgical-fix mode uses, borrowed without its entry conditions, which are defined over plan-review rounds and do not apply here — and records the before/after text in the phase-fit record, which is the edit's audit trail until the next decision revision, whose charter review reads the accumulated administrative edits as part of the artifact. A finding that supplies no replacement text, or an edit that would change a decision, is a decision revision whatever it is called; when in doubt, it is a decision.
...
The implementation executor edits only its `SCOPE_PATHS` list — an admitted-class site outside it is a stop-and-return the orchestrator answers by extending the list and re-dispatching, never by a planner round — and runs **preparatory** checks. Preparatory success is not completion evidence. Its existing discriminating-test, selected-authority, coverage-honesty, maintainer-simulation, and CR-annotation gates remain the authoritative gates; do not restate or replace them here.
