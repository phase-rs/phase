...
These terminal stops join the unbounded loops' enumerated stop-condition list.
...
- T4 fired and the charter-mode planner returned a feasibility exit (no green seam),
- T4 fired with a prior T4 firing or feasibility exit already in the phase-fit record, or
- T5 fired at this loop's closure, per "T5 — the loop counter".

The T4 conditions above stop the run and surface to the user with the phase-fit record; a T5 firing surfaces with the choice T5 names. A T4 firing without those conditions exits this loop into decomposition per "Phase-fit gate and chartered runs" — that is a route, not a stop.
