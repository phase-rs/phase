...
When the user explicitly asks to "babysit", "watch", "monitor", or "keep an eye on" a PR — or when CI is slow and bots (Gemini, Sentry) routinely post late — enter babysit mode after pushing fixes. The goal is to detect new comments and CI failures and react before the maintainer has to.
...
After exit, proceed to Labeling + Enqueue. The two-quiet-cycles rule exists specifically because bot reviewers (Gemini, Sentry, others) routinely post late and a premature enqueue means racing a fresh `CHANGES_REQUESTED` review.
