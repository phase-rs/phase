...
2. **Teammates cannot spawn subagents.** All review subagents must be spawned by the lead. The lead receives the plan/implementation from the teammate, spawns a review subagent (model: opus), and sends feedback back to the teammate. **Commit the implementation as soon as it compiles and its own tests pass, before starting this review loop** — the loop is not a safe place to leave work staged, since an interruption mid-loop then means re-deriving trust in whatever's still uncommitted instead of just resuming. Scale the round cap to the change's footprint rather than defaulting to the same number every time: **one round** for a routine, single-file/single-mechanic change; **up to three rounds** for a genuinely cross-cutting change (a new building block spanning parser + resolver + targeting, or similar multi-module surface). Decide the cap up front, not implicitly as rounds accumulate. If the cap is reached without a clean review, stop and record the remaining findings for the PR body rather than continuing to iterate — an open-ended "until clean" loop with no enforced ceiling is what let one review consume an entire card's token budget (~497k tokens, ~225 tool calls) and land zero commits.
