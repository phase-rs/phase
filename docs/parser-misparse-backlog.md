...
- **Distinct cards implicated:** 4707
- **Total card appearances across root causes:** 4740 (a card may appear under more than one root cause when it exhibits multiple distinct misparses)
...
| 6 | Disjunctive (or-list) collapsed to first branch | 234 | oracle_nom/filter.rs + oracle_target.rs — build TargetFilter::Or across all alt() branches |
...
### 6. Disjunctive (or-list) collapsed to first branch  (234 cards)
