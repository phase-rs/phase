...
same-origin `/card-image-art` and `/card-image-back` routes. Vite provides those
routes in development, and the Cloudflare Pages functions under
`client/functions/` provide them for the hosted client. Other deployments must
map those two paths to `cards.scryfall.io` and `backs.scryfall.io` respectively.
