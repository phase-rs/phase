...
mod tests {
    ...
    // CR 510.1a + CR 120.3c + CR 613.4c: A creature whose power is enhanced by
    // +1/+1 counters (layer 7c) must deal combat damage equal to its
    // LAYER-EVALUATED power, not its base power. Regression test for issue #2352:
    // a 9/11 creature (base 1/3, 8 +1/+1 counters) attacked a planeswalker and
    // only removed 1 loyalty counter (the base power) instead of 9 (evaluated power).
    // The root cause: layers_dirty was marked full (e.g. by commit_attackers at
    // combat.rs:2045) but flush_layers runs in run_post_action_pipeline AFTER
    // resolve_combat_damage executes inside handle_priority_pass → auto_advance.
    // Fix: turns.rs Phase::CombatDamage now calls flush_layers before
    // resolve_combat_damage so combat_damage_amount reads evaluated power.
    #[test]
    fn attacker_pt_counters_deal_full_power_damage_to_planeswalker() {
        use crate::game::combat::AttackTarget;

        let mut state = setup();

        // Create a 1/3 creature with 8 +1/+1 counters: layer-evaluated power = 9.
        let attacker = create_creature(&mut state, PlayerId(0), "Counter Beast", 1, 3);
        {
            let obj = state.objects.get_mut(&attacker).unwrap();
            // Set base characteristics so evaluate_layers uses the correct base.
        ...
        let pw = create_planeswalker(&mut state, PlayerId(1), "Professor Onyx", 10);

        // Reproduce the bug: mark layers dirty WITHOUT flushing first, simulating
        // the production path where commit_attackers marks dirty but flush_layers
        // runs after resolve_combat_damage returns (too late).
        state.layers_dirty.mark_full();
        // obj.power is still Some(1) (base power; counters not yet reflected).
        assert_eq!(
            state.objects.get(&attacker).unwrap().power,
            Some(1),
            "pre-condition: power must be stale (base only) before resolve_combat_damage"
        );
        ...
        // Simulate the production path from turns.rs Phase::CombatDamage, which
        // calls flush_layers before resolve_combat_damage so combat_damage_amount
        // reads layer-evaluated power (base + counters).
        crate::game::layers::flush_layers(&mut state);

        let mut events = Vec::new();
        resolve_combat_damage(&mut state, &mut events);

        // CR 510.1a + CR 120.3c + CR 613.4c: damage = evaluated power = 9; removes 9
        // loyalty counters (10 → 1). Before the fix, stale base power (1) was used.
        let pw_obj = state.objects.get(&pw).unwrap();
        assert_eq!(
            pw_obj.loyalty,
            Some(1),
            "planeswalker should have 1 loyalty after 9 damage (10 - 9); got {:?}",
            pw_obj.loyalty
        );
        assert_eq!(state.players[1].life, 20, "player life should be unchanged");
    }
