...
fn filter_refs_same_name_as_parent_target(filter: &TargetFilter) -> bool {
    match filter {
        ...
        TargetFilter::Or { filters } | TargetFilter::And { filters } => filters
            .iter()
            .any(filter_refs_same_name_as_parent_target),
...
mod tests {
    ...
    use crate::types::ability::{
        ...
        QuantityRef, SpellContext, StaticDefinition, TargetFilter, TargetRef, TargetSelectionMode,
    ...
    /// Issue #2403: Sin, Spira's Punishment — random exile must publish the
    /// tracked set consumed by the chained `CopyTokenOf { TrackedSet(0) }`.
    #[test]
