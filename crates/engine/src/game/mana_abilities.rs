...
fn apply_condition_instead_mana_swap(
    ...
    if super::effects::evaluate_condition(inner, state, ability) {
        if matches!(sub.effect, Effect::Mana { target: None, .. }) {
            if mana_instead_should_replace_base_with_sub(ability, sub, inner) {
                return super::ability_utils::apply_instead_swap(ability, sub);
            }
            // CR 614.1a + issue #1299: Same-kind colorless deltas (e.g. Ugin's
            // Labyrinth "add {C}{C} instead") normalize to base + delta. If
            // `ConditionInstead` survived on the sub, swapping would collapse
            // production to the delta alone — keep the base and let the sub
            // chain add the bonus colorless inline.
            return ability.clone();
        }
        return ability.clone();
    }
...
/// CR 614.1a: Whether a satisfied `ConditionInstead` mana sub-ability fully
/// replaces the parent's production (`apply_instead_swap`) rather than adding
/// to it via the sub_ability chain.
fn mana_instead_should_replace_base_with_sub(
    ...
    ) = (&parent.effect, &sub.effect)
    else {
        return true;
    };

    if std::mem::discriminant(parent_produced) != std::mem::discriminant(sub_produced) {
        // Different production kinds (e.g. Gemstone Caverns colorless → any
        // color) are true replacements, not additive deltas.
        return true;
    }
    ...
    let (
        ...
        ManaProduction::Colorless {
            count: QuantityExpr::Fixed { value: sub_count },
        },
    ) = (parent_produced, sub_produced)
    else {
        // Non-fixed or non-colorless same-kind clauses keep legacy swap
        // semantics until a parameterized pattern is needed.
        return true;
    };

    if *sub_count > *base_count {
        // Pre-normalize full replacement ("add {C}{C}{C} instead" on a {C}
        // base) — the sub carries the total replacement amount.
        return true;
    }

    if *sub_count == *base_count && !condition_reads_cards_exiled_by_source(inner) {
        // Equal replacement ("add {C} instead") — swap, not additive.
        return true;
    }

    // Delta-shaped same-kind colorless (issue #1299) or linked-exile riders at
    // the same increment as the base.
    false
}

fn condition_reads_cards_exiled_by_source(condition: &AbilityCondition) -> bool {
    match condition {
        ...
        AbilityCondition::Not { condition } => condition_reads_cards_exiled_by_source(condition),
        AbilityCondition::And { conditions } | AbilityCondition::Or { conditions } => conditions
            .iter()
            .any(condition_reads_cards_exiled_by_source),
        _ => false,
    }
}
...
mod tests {
    ...
    /// CR 406.6 + CR 614.1a: issue #1299 — Ugin's Labyrinth produces two
    /// colorless when a card is imprinted (linked in `exile_links`).
    #[test]
    fn ugin_labyrinth_imprint_exile_populates_links_then_mana_produces_two_colorless() {
        use crate::types::ability::{
            AbilityCondition, Comparator, Effect as Ef, QuantityExpr, QuantityRef, ResolvedAbility,
            TargetFilter, TargetRef,
        };

        let mut state = GameState::new_two_player(42);
        let labyrinth = create_object(
            &mut state,
            CardId(3000),
            PlayerId(0),
            "Ugin's Labyrinth".to_string(),
            Zone::Battlefield,
        );
        state
            .objects
            ...
            .core_types
            .push(crate::types::card_type::CoreType::Land);
        Arc::make_mut(&mut state.objects.get_mut(&labyrinth).unwrap().abilities).push(
            AbilityDefinition::new(
                AbilityKind::Activated,
                Ef::Mana {
                    produced: ManaProduction::Colorless {
        ...
        state.objects.get_mut(&imprinted).unwrap().color.clear();
        ...
        let etb = ResolvedAbility::new(
            ...
            vec![TargetRef::Object(imprinted)],
            labyrinth,
            PlayerId(0),
        );
        let mut events = Vec::new();
        ...
        assert!(
            state
                .exile_links
                .iter()
                .any(|link| { link.source_id == labyrinth && link.exiled_id == imprinted }),
            "imprint exile must populate exile_links"
        );

        let mana_def = state.objects.get(&labyrinth).unwrap().abilities[0].clone();
        let mut mana_events = Vec::new();
        resolve_mana_ability(
            &mut state,
            labyrinth,
    ...
    /// CR 614.1a: When the parser has not yet normalized `ConditionInstead` on
    /// the delta sub-ability, `apply_condition_instead_mana_swap` must still
    /// net to base + delta — not replace base production with the delta alone.
    #[test]
    fn ugin_labyrinth_condition_instead_delta_still_produces_two_colorless() {
        use crate::types::ability::{
            AbilityCondition, Comparator, Effect as Ef, QuantityExpr, QuantityRef,
        };
        ...
        let exiled = create_object(
            ...
            CardId(3011),
            PlayerId(0),
            "Imprinted Card".to_string(),
            Zone::Exile,
        );
        crate::game::exile_links::push_tracked_by_source(&mut state, exiled, labyrinth);
        ...
        assert_eq!(
            state.players[0].mana_pool.count_color(ManaType::Colorless),
            2,
            "ConditionInstead delta must not collapse to 1 colorless when imprinted"
        );
    }
