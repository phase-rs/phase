...
pub(super) fn handle_resolution_choice(
    ...
    let outcome = match (waiting_for, action) {
        ...
        (
            ...
            // CR 119.7-8: apply the chosen assignment. Every enumerated option is
            // already legal (the resolver filtered CR 119.7/119.8 per receiver).
            ...
            match effects::life::apply_life_totals_assignment(state, &assignment, events)
                .map_err(|err| EngineError::InvalidAction(err.to_string()))?
            {
                // CR 614.7: a competing replacement installed a choice WaitingFor;
