...
pub(super) fn handle_resolution_choice(
    ...
    let outcome = match (waiting_for, action) {
        ...
        (
            ...
            // CR 607.2d + CR 613.1 + CR 608.2c: Persist the chosen attribute (if
            // any) on the source, recompute layers it gates, and set
            // `last_named_choice`. Single authority shared with the random
            // `Choose` resolver so both modes record identically (Morophon
            // buffs, Pithing Needle prohibitions, Sewer Nemesis CDA, …).
            effects::choose::record_resolution_choice(state, &choice_type, &choice, source_id);
