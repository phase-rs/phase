...
use super::oracle_static::{
    parse_commander_subject_filter_prefix, parse_subtype_or_list_insensitive_prefix,
};
            ...
            let (type_part, cast_origin, origin_tail_ok) =
                match nom_primitives::split_once_on(trimmed, " from ") {
                    Ok((_, (before, from_tail))) => {
                        ...
                        match parse_cast_origin_zone(origin_text.trim_start()) {
                            Ok((rest, Some(origin))) if rest.trim().is_empty() => {
                                (before, OriginConstraint::Equals(origin), true)
                            }
                            Ok((rest, None)) if rest.trim().is_empty() => {
                                (before, OriginConstraint::Any, true)
                            }
...
fn parse_cast_type_list_filter(type_phrase: &str) -> (TargetFilter, &str) {
    ...
    parse_subtype_or_list_insensitive_prefix(type_phrase).unwrap_or((filter, remainder))
}
