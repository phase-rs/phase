...
#[test]
fn static_crew_contribution_power_delta_self_ref() {
    // CR 702.122c: Reckoner Bankbuster / Shorikai class.
    let def = parse_static_line("~ crews Vehicles as though its power were 2 greater.").unwrap();
    assert_eq!(
        def.mode,
...
#[test]
fn static_crew_contribution_saddle_and_crew_combo() {
    // CR 702.171 + CR 702.122: Cloudspire Captain class.
    let def = parse_static_line(
...
#[test]
fn static_crew_contribution_toughness_substitution() {
    // CR 702.122c: Giant Ox class.
    let def =
        parse_static_line("~ crews Vehicles using its toughness rather than its power.").unwrap();
    assert_eq!(
        def.mode,
        StaticMode::CrewContribution {
...
#[test]
fn static_crew_contribution_controlled_creatures_grant() {
    // CR 702.122c + CR 702.184: Stoic Star-Captain class.
    let def = parse_static_line(
