...
/// indicates", so a bare anaphoric pronoun ("it" / "them" / "him" / "her") in a
/// per-recipient continuous static names **the object currently receiving the
/// effect**, not the ability's source. `parse_counter_object_scope`
/// (`oracle_nom/quantity.rs`) has no view of the enclosing static, so it binds
/// every counter anaphor to `ObjectScope::Source`; this post-parse pass rebinds
/// it to `ObjectScope::Recipient` once the affected set is known.
///
...
/// the modification never applied.
///
/// The rebind is gated on the **pronoun form**, not merely on the affected set:
/// the far larger Door of Destinies / Joraga Warcaller / Lion Sash class
/// ("…for each charge counter on ~") names the source explicitly, and there
/// `Source` is already correct. Only a bare anaphor defers to the recipient.
pub(crate) fn bind_counter_anaphor_to_recipient(def: &mut StaticDefinition) {
    // CR 613.4c: only layer-evaluated continuous statics have a per-object
    // recipient to bind. Every other `StaticMode` resolves once, source-scoped.
    if !matches!(def.mode, StaticMode::Continuous) {
        return;
    }
    // CR 613.4c: a self-referential subject IS the recipient, so `Source` is
    // already the right binding (Earthen Goo: "~ gets +1/+1 for each age
    // counter on it"). Leave it alone rather than churn an equivalent scope.
    if affected_names_the_source(def.affected.as_ref()) {
        return;
    }
    // No description ⇒ no text to prove the pronoun form from. Stay source-bound
    // rather than guess; an unproven rebind would silently break the far larger
    // explicit-`~` class.
    let Some(description) = def.description.as_ref().map(|d| d.to_lowercase()) else {
        return;
    };
    if !description_binds_counter_anaphor(&description) {
        return;
    }
    for modification in def.modifications.iter_mut() {
        if let Some(value) = continuous_modification_dynamic_quantity_mut(modification) {
            rebind_source_counters_to_recipient(value);
        }
    }
}
...
/// CR 611.3a: Scan word boundaries for a counter **quantity** phrase that ends
/// in a bare anaphoric pronoun. Uses the multi-position phrase-scan idiom (cf.
/// `scan_timing_restrictions`) because the phrase can sit anywhere in the line.
fn description_binds_counter_anaphor(lower: &str) -> bool {
    let mut remaining = lower;
    while !remaining.is_empty() {
        if parse_counter_quantity_anaphor(remaining).is_ok() {
            return true;
        }
        remaining = match remaining.find(' ') {
            Some(index) => remaining[index + 1..].trim_start(),
            None => "",
        };
    }
    false
...
/// CR 122.1 + CR 611.3a: `"for each" | "the number of"` … `"counter(s) on"` …
/// bare pronoun — the counter-scaling grammar whose object is deferred.
///
/// The span between the quantity introducer and the counter noun may not cross
/// a sentence break or a quoted granted ability (`"`), so a card whose own
/// counter phrase names `~` cannot be matched through a *granted* ability's
/// inner "put a counter on it" (Rakdos Riteknife class).
fn parse_counter_quantity_anaphor(input: &str) -> OracleResult<'_, ()> {
    let (input, _) =
        alt((tag("for each "), tag("the number of "), tag("number of "))).parse(input)?;
    let (input, span) = take_until::<_, _, OracleError<'_>>("counter").parse(input)?;
    if span.contains(['"', '.', ';', ':']) {
    ...
    let (input, _) = alt((tag("counters on "), tag("counter on "))).parse(input)?;
    let (input, _) = parse_bare_object_pronoun(input)?;
    Ok((input, ()))
}

/// CR 611.3a: The bare objective pronouns that defer their referent. `~` and
/// "this creature" are deliberately absent — those name the source outright.
/// The trailing word-boundary peek keeps "it" from matching the head of "its".
fn parse_bare_object_pronoun(input: &str) -> OracleResult<'_, ()> {
    let (input, _) = alt((tag("it"), tag("them"), tag("him"), tag("her"))).parse(input)?;
    let (input, _) = peek(alt((eof, tag("."), tag(","), tag(";"), tag(" ")))).parse(input)?;
    Ok((input, ()))
}
...
/// CR 122.1: Rebind every `CountersOn { scope: Source }` read inside a dynamic
/// magnitude to the recipient. Only the counter axis moves — a sibling
/// `Power { scope: Source }` in the same expression keeps naming the source,
/// because only the counter phrase was proven anaphoric by the scan above.
fn rebind_source_counters_to_recipient(expr: &mut QuantityExpr) {
    match expr {
        QuantityExpr::Ref {
            qty: QuantityRef::CountersOn { scope, .. },
        } if *scope == ObjectScope::Source => *scope = ObjectScope::Recipient,
        QuantityExpr::Ref { .. } | QuantityExpr::Fixed { .. } => {}
        QuantityExpr::DivideRounded { inner, .. }
        | QuantityExpr::Offset { inner, .. }
        | QuantityExpr::ClampMin { inner, .. }
        | QuantityExpr::Multiply { inner, .. } => rebind_source_counters_to_recipient(inner),
        QuantityExpr::UpTo { max } => rebind_source_counters_to_recipient(max),
        QuantityExpr::Power { exponent, .. } => rebind_source_counters_to_recipient(exponent),
        QuantityExpr::Difference { left, right } => {
            rebind_source_counters_to_recipient(left);
            rebind_source_counters_to_recipient(right);
        }
        QuantityExpr::Sum { exprs } | QuantityExpr::Max { exprs } => exprs
            .iter_mut()
            .for_each(rebind_source_counters_to_recipient),
    }
}
