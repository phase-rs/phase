...
fn parse_cast_permission_additional_cost_rider(
    ...
    // each an independent (verb-prefix × quantity × unit) axis. Recognizes both
    // "by paying <N> life" (Festival of Embers) and "by discarding <N> card(s)"
    // (Me, the Immortal); the generalized head also subsumes upstream's inline
    // "by paying <N> life" shape.
    ...
    // CR 601.2f: "paying " is optional — Festival of Embers omits it ("in addition
    // to their other costs"), while Me, the Immortal / Noctis, Prince of Lucis
    // include it ("in addition to PAYING its other costs"). Both denote the
    // spell's normal costs paid alongside. Additive `opt` strip: Festival (no
    // gerund) keeps the original slice unchanged.
    let after_paying =
