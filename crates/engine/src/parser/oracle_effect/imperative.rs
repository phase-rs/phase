...
pub(super) fn parse_shuffle_ast(text: &str, lower: &str) -> Option<ShuffleImperativeAst> {
        ...
        nom_on_lower(text, lower, |input| value((), tag("shuffle ")).parse(input))
    {
            ...
            && !nom_primitives::scan_contains(lower, "from")
        {
            ...
            // Guard: the target must be a typed filter with meaningful content
            // (type_filters or properties), AND the remainder after the target
            // must start with "into". This prevents false matches on zone
            // references like "their hand and graveyard into their library".
            let is_meaningful_typed = matches!(
                &target,
                TargetFilter::Typed(tf) if !tf.type_filters.is_empty() || !tf.properties.is_empty()
            );
            if is_meaningful_typed && remainder_lower.starts_with("into") {
                let owner_library = nom_primitives::scan_at_word_boundaries(lower, |input| {
                    alt((
                        value((), tag::<_, _, OracleError<'_>>("its owner's library")),
                        value((), tag("their owner's library")),
