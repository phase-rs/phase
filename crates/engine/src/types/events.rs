...
impl EventObjectSnapshot {
    ...
    pub fn classify_filter_shape(filter: &TargetFilter) -> EventObjectFilterSupport {
        ...
        match filter {
            ...
            // CR 406.6 + CR 607.2a: "played/cast from among cards exiled with ~"
            // (The Matrix of Time). Not reachable from the connive-style subject
            // grammar — attached only to LandPlayed/SpellCast triggers via a
            // dedicated gate — and answering it reads the object's
            // `played_from_exile_source` provenance stamp plus the trigger source
            // id out of live state, which this snapshot deliberately omits.
