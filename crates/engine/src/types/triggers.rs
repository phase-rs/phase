...
mod tests {
    ...
    fn parse_known_trigger_modes() {
        assert_eq!(TriggerMode::from_str("ChangesZone").unwrap(), TriggerMode::ChangesZone);
        assert_eq!(TriggerMode::from_str("DamageDone").unwrap(), TriggerMode::DamageDone);
        assert_eq!(TriggerMode::from_str("SpellCast").unwrap(), TriggerMode::SpellCast);
        assert_eq!(TriggerMode::from_str("Attacks").unwrap(), TriggerMode::Attacks);
        assert_eq!(TriggerMode::from_str("Blocks").unwrap(), TriggerMode::Blocks);
        assert_eq!(TriggerMode::from_str("AttackerBlocked").unwrap(), TriggerMode::AttackerBlocked);
        assert_eq!(TriggerMode::from_str("LifeGained").unwrap(), TriggerMode::LifeGained);
        assert_eq!(TriggerMode::from_str("TokenCreated").unwrap(), TriggerMode::TokenCreated);
    }
    ...
    fn trigger_mode_count_at_least_137() {
        ...
        let forge_modes = [
            "Abandoned", "AbilityCast", "AbilityResolves", "AbilityTriggered",
            "Adapt", "Airbend", "Always", "Attached",
            "AttackerBlocked", "AttackerBlockedOnce", "AttackerBlockedByCreature",
            "AttackersDeclared", "AttackersDeclaredOneTarget",
            "AttackerUnblocked", "AttackerUnblockedOnce", "Attacks",
            "BecomeMonarch", "BecomeMonstrous", "BecomeRenowned",
            "BecomesCrewed", "BecomesPlotted", "BecomesSaddled",
            "BecomesTarget", "BecomesTargetOnce",
            "BlockersDeclared", "Blocks",
            "CaseSolved", "Championed", "ChangesController",
            "ChangesZone", "ChangesZoneAll", "ChaosEnsues",
            "ClaimPrize", "Clashed", "ClassLevelGained",
            "CommitCrime", "ConjureAll", "CollectEvidence",
            "CounterAdded", "CounterAddedOnce", "CounterPlayerAddedAll",
            "CounterTypeAddedAll", "CounterAddedAll",
            "Countered", "CounterRemoved", "CounterRemovedOnce",
            "CrankContraption", "Crewed", "Cycled",
            "DamageAll", "DamageDealtOnce", "DamageDone",
            "DamageDoneOnce", "DamageDoneOnceByController", "DamagePreventedOnce",
            "DayTimeChanges", "Destroyed", "Devoured",
            "Discarded", "DiscardedAll", "Discover",
            "Drawn", "DungeonCompleted",
            "Earthbend", "ElementalBend", "Enlisted",
            "Evolved", "ExcessDamage", "ExcessDamageAll",
            "Exerted", "Exiled", "Exploited", "Explores",
            "Fight", "FightOnce", "Firebend", "FlippedCoin",
            "Forage", "Foretell", "FullyUnlock", "GiveGift",
            "Immediate", "Investigated",
            "LandPlayed", "LifeGained", "LifeLost", "LifeLostAll", "LosesGame",
            "ManaAdded", "ManaExpend", "ManifestDread",
            "Mentored", "Milled", "MilledOnce", "MilledAll",
            "Mutates", "NewGame",
            "PayCumulativeUpkeep", "PayEcho", "PayLife",
            "Phase", "PhaseIn", "PhaseOut", "PhaseOutAll",
            "PlanarDice", "PlaneswalkedFrom", "PlaneswalkedTo",
            "Proliferate", "Revealed", "RingTemptsYou",
            "RolledDie", "RolledDieOnce", "RoomEntered",
            "Saddled", "Sacrificed", "SacrificedOnce",
            "Scry", "SearchedLibrary", "SeekAll", "SetInMotion", "Shuffled",
            "Specializes", "SpellAbilityCast", "SpellAbilityCopy",
            "SpellCast", "SpellCastOrCopy", "SpellCopy",
            "Stationed", "Surveil", "TakesInitiative",
            "TapAll", "Taps", "TapsForMana",
            "TokenCreated", "TokenCreatedOnce",
            "Trains", "Transformed", "TurnBegin", "TurnFaceUp",
            "Unattach", "UnlockDoor", "UntapAll", "Untaps",
            "VisitAttraction", "Vote", "Waterbend",
        ];
