...
/// Legacy card-data compatibility: replacements parsed before the SelfRef
/// annotation still fire when hosted on the resolving stack spell.
#[test]
fn nexus_shuffle_back_without_selfref_annotation_still_redirects() {
    let mut scenario = GameScenario::new();
    scenario.at_phase(Phase::PreCombatMain);
    let nexus = scenario
    ...
    let mut runner = scenario.build();
    {
        let obj = runner.state_mut().objects.get_mut(&nexus).unwrap();
        let mut repls: Vec<_> = obj.replacement_definitions.as_slice().to_vec();
        for repl in &mut repls {
            repl.valid_card = None;
        }
        obj.replacement_definitions = repls.into();
    }

    let _ = runner.cast(nexus).resolve();

    assert_eq!(
        runner.state().objects[&nexus].zone,
        Zone::Library,
        "legacy valid_card=None stack-hosted shuffle-back must still redirect"
    );
    assert!(
        !runner.state().players[0].graveyard.contains(&nexus),
        "legacy shape must not leave the spell in the graveyard"
    );
}
