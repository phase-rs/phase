...
//! `AbilityBlockEntry { ability_index, source, kind }` per blocked activated
//! ability, in enforcement-gate order (CantBeActivated → CantActivateDuring →
...
use engine::types::ability::{
    AbilityBlockKind, AbilityCost, AbilityDefinition, AbilityKind, AbilityTag, ChosenAttribute,
    Effect, GameRestriction, ManaContribution, ManaProduction, ProhibitedActivity, QuantityExpr,
    RestrictionExpiry, RestrictionPlayerScope, TargetFilter,
};
...
fn blocked(runner: &GameRunner, id: ObjectId) -> Vec<(usize, ObjectId, AbilityBlockKind)> {
    runner.state().objects[&id]
        .blocked_abilities
        .iter()
        .map(|e| (e.ability_index, e.reason.source, e.reason.kind))
        .collect()
}
...
fn pithing_needle_blocks_nonmana_exempts_mana() {
    ...
    assert_eq!(
        ...
        vec![(0, needle, AbilityBlockKind::CantBeActivated)],
...
fn immortal_sun_blocks_loyalty_only() {
    ...
    assert_eq!(
        ...
        vec![(0, sun, AbilityBlockKind::CantBeActivated)],
...
fn city_of_solitude_blocks_non_active_players_ability() {
    ...
    assert_eq!(
        ...
        vec![(0, city, AbilityBlockKind::CantActivateDuring)],
...
fn kang_prohibit_activity_tag_scoped() {
    ...
    assert_eq!(
        ...
        vec![(0, kang, AbilityBlockKind::Prohibited)],
...
/// still records `Prohibited` with `reason.source == <departed id>`, and that id
/// is no longer present in `state.objects` (the frontend departed-source guard is
/// exercised in the component test).
#[test]
