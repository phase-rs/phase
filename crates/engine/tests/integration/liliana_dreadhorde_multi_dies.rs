...
fn liliana_dreadhorde_general_draws_once_per_simultaneous_death() {
    ...
    // Fire dies triggers from those SBA events. CR 603.3b: when one player
    // controls multiple simultaneously-triggered abilities, the engine asks
    // the player to choose an ordering. Drain the resulting `OrderTriggers`
    // prompt with the identity order — every permutation is observably
    // equivalent here (no targets, same effect, distinct trigger_event per
    // dying creature), so the prompt is immaterial for the regression under
    // test (three independent trigger instances landing on the stack).
