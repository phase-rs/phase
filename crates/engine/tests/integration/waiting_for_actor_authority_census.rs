...
fn every_waiting_for_arm_declares_its_acting_authority() {
    ...
    // 136 -> 137 is adjudicated: Dig rest "in any order" added `DigBottomOrder`.
    // It names one acting `player` (CR 608.2d: the ability controller announces
    // the order) and is classified by `WaitingFor::acting_authority` as
    // `ActingAuthority::One(player)` — not actorless.
    if declared.len() != 137 {
        failures.push(format!(
