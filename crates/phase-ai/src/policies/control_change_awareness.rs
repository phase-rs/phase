...
mod tests {
    ...
    use engine::types::ability::{
        AbilityDefinition, AbilityKind, ControllerRef, Effect, TargetFilter, TypedFilter,
    };
    use engine::types::card_type::CoreType;
    ...
    /// Test that the policy penalizes giving away own permanents via GainControl
    #[test]
    fn penalizes_gain_control_of_own_permanent() {
        ...
        // Create an ability that gives control to opponent
        ...
        let policy = ControlChangeAwarenessPolicy;
        let verdict = policy.verdict(&ctx);

        // Should penalize giving away own permanent
        assert!(
            matches!(verdict, PolicyVerdict::Score { delta, .. } if delta == CONTROL_CHANGE_PENALTY)
        );
    }

    /// Test that the policy does not penalize gaining control of opponent permanents
    #[test]
    fn does_not_penalize_gain_control_of_opponent_permanent() {
        let mut state = GameState::default();
        let mut context = AiContext::default();
        let config = AiConfig::default();

        // Create a creature controlled by opponent
        let creature_id = ObjectId(1);
        let creature_obj = create_object(
            creature_id,
            CardId(100),
        ...
        // Create an ability that gives control to AI
        let ability = AbilityDefinition {
            kind: AbilityKind::Activated,
            effect: Box::new(Effect::GainControl {
                target: TargetFilter::Typed(
        ...
        // Should not penalize gaining opponent's permanent
