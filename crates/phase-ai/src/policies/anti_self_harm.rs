...
fn optional_effect_life_cost(ctx: &PolicyContext<'_>, source_id: ObjectId) -> Option<i32> {
    ...
    obj.replacement_definitions
        ...
        .filter_map(|effect| match effect {
            Effect::LoseLife { amount, .. } => Some(resolve_life_amount(ctx, amount, source_id)),
            _ => None,
        })
        .max()
}

/// Resolve a `LoseLife` amount to a concrete life payment for self-harm evaluation.
/// `Fixed` amounts are exact; dynamic amounts resolve against the current game state
/// via the engine's `resolve_quantity` (the AI controls the source). Resolution is
/// clamped to be non-negative so a 0 / negative dynamic value never registers as a cost.
fn resolve_life_amount(ctx: &PolicyContext<'_>, amount: &QuantityExpr, source_id: ObjectId) -> i32 {
    match amount {
        QuantityExpr::Fixed { value } => *value,
        _ => resolve_quantity(ctx.state, amount, ctx.ai_player, source_id).max(0),
    }
}
...
mod tests {
    ...
    use engine::types::ability::{ReplacementDefinition, ReplacementMode};
    ...
    fn optional_effect_accept_score(state: &GameState, source_id: ObjectId) -> f64 {
        let config = AiConfig::default();
        let decision = AiDecisionContext {
            waiting_for: WaitingFor::Priority {
                player: PlayerId(0),
    ...
    fn optional_life_cost_in_else_branch_penalises_lethal_accept() {
        ...
        let score = optional_effect_accept_score(&state, source_id);
        assert!(
            score <= -100.0,
    ...
    fn optional_life_cost_non_fixed_amount_resolves_and_penalises() {
        let mut state = make_state();
        // Three AI creatures → "for each creature you control" resolves to 3.
        add_creature(&mut state, PlayerId(0), "Bear", 2, 2);
        add_creature(&mut state, PlayerId(0), "Bear", 2, 2);
        add_creature(&mut state, PlayerId(0), "Bear", 2, 2);
