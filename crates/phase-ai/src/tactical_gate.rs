...
mod tests {
    ...
    #[test]
    fn attacking_only_removal_requires_a_live_attacker() {
        let mut scenario = GameScenario::new();
        let attacker = scenario.add_creature(P1, "Attacker", 2, 2).id();
        scenario.add_creature(P1, "Nonattacker", 2, 2);
        ...
        let cast = CandidateAction {
            action: GameAction::CastSpell {
                object_id: removal,
                card_id: state.objects.get(&removal).unwrap().card_id,
                targets: Vec::new(),
                payment_mode: CastPaymentMode::Auto,
            },
            metadata: ActionMetadata {
        ...
        let candidates = vec![cast.clone(), pass.clone()];
        let config = create_config(AiDifficulty::VeryHard, Platform::Wasm);
        ...
        let gated = gate_candidates(state, &decision, candidates.clone(), P0, &config, &context);
        assert!(
            !gated
                .iter()
                .any(|entry| entry.candidate.action == cast.action),
            "attacking-only removal must be gated when no opposing creature is attacking"
        );
        assert!(
            gated
                .iter()
                .any(|entry| entry.candidate.action == pass.action),
            "the paired fixture must retain Pass"
        );
        ...
        let gated = gate_candidates(state, &decision, candidates, P0, &config, &context);

        assert!(
            gated
                .iter()
                .any(|entry| entry.candidate.action == cast.action),
            "the identical spell must survive gating once a legal attacker exists"
        );
        assert!(
            gated
                .iter()
                .any(|entry| entry.candidate.action == pass.action),
            "the paired fixture must still retain Pass"
        );
    }
