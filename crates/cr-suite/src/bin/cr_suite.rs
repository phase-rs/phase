            ...
            "--format" => {
                if let Some(v) = iter.next() {
                    format = match v.as_str() {
                        ...
                        other => {
                            eprintln!("unknown --format {other:?} (use plain|md|json)");
                            process::exit(2);
                        }
                    };
                }
...
Options:
  ...
  --format FMT            Run output format: plain (default) | md | json
"
    );
}
...
fn do_run(config: &Config) -> Result<bool, String> {
    ...
    match config.format {
        OutputFormat::Plain => print!("{}", report.render_summary()),
        OutputFormat::Markdown => print!("{}", report.render_markdown()),
