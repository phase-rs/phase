...
//! That nested payload can carry retained `spectator_password`, `config.rng_seed`,
//! and unopened `packs_by_seat`, which must be stripped before persistence or GET echo.
...
/// Host snapshot field carrying a serialized persisted draft session.
...
const NESTED_DRAFT_SECRET_KEYS: &[&str] = &["packs_by_seat", "spectator_password"];
...
fn redact_draft_session_object(session: &mut Map<String, Value>) {
    ...
    if let Some(Value::Object(lobby)) = session.get_mut("lobby_meta") {
        lobby.remove("password");
    }
}
...
mod tests {
    ...
    #[test]
    fn redact_p2p_backup_snapshot_secrets_strips_nested_spectator_password() {
        let nested = serde_json::json!({
            "draft_code": "ABC123",
            "spectator_password": "secret",
            "lobby_meta": { "host_name": "Alice", "password": "secret", "public": false },
            "config": { "rng_seed": 42 },
            "packs_by_seat": [[{"card_id": "hidden"}]],
            "status": "Drafting"
        });
        ...
        assert!(nested_out.get("spectator_password").is_none());
        assert!(nested_out.get("packs_by_seat").is_none());
        assert_eq!(nested_out["config"]["rng_seed"], 0);
        assert!(nested_out["lobby_meta"].get("password").is_none());
